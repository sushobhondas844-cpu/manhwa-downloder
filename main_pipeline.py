...
def get_google_services():
    """Authenticates using personal User OAuth tokens from environment variables to bypass storage quotas."""
...
def create_resilient_session():
    """Builds a robust HTTP session with automatic retries and browser emulation to prevent connection drops."""
    ...
    session.headers.update({
        "User\x2dAgent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
...
def extract_panel_urls(page_html, base_url):
    """Parses webpage structures using JSON blocks and DOM tags to identify high resolution image links."""
    ...
    reader_container = soup.select_one("#readerarea") or soup.select_one(".reading\x2dcontent") or soup.select_one(".entry\x2dcontent") or soup.select_one("div[id*='image\x2dcontainer']") or soup
    lazy_attributes = ["data\x2dsrc", "data\x2dlazy\x2dsrc", "data\x2doriginal", "data\x2durl", "src"]
...
def verify_and_save_image(image_bytes, target_path, min_size_kb=15):
    """Validates image bytes utilizing Pillow to ensure no corrupted files are saved locally."""
...
def upload_folder_to_drive(drive_service, local_folder, folder_name, parent_id):
    """Pushes the verified local images into your Google Drive using your personal storage allocation."""
    file_metadata = {
        'name': folder_name,
...
def process_queue():
    """Orchestrates the entire workflow from reading the Google Sheet to downloading and uploading."""
    ...
    for idx, row in enumerate(records, start=2):
        ...
        if success_count > 0:
            ...
            uploaded_id = upload_folder_to_drive(drive_service, local_dir, gdrive_folder_name, RAW_STAGING_FOLDER_ID)
